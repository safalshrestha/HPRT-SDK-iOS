...
- (int)printContent
{
    ...
    ret = [myApp.myPos POS_RTQueryStatus:1 status:&status timeout:3000 MaxRetry:2];
    ...
    if (ret)
        ret = [myApp.myLabel PageBegin:0 starty:0 width:384 height:500 rotate:0];
    if (ret)
        ret = [myApp.myLabel DrawPlainText:12 starty:40 font:24 style:0 str:(char *)[@"测试" cStringUsingEncoding:enc]];
    if (ret)
        ret = [myApp.myLabel DrawBarcode:12 starty:100 type:8 height:50 unitwidth:2 rotate:0 str:"NO.20160423"];
    if (ret)
        ret = [myApp.myLabel DrawQRCode:12 starty:200 version:0 ecc:4 unitwidth:4 rotate:0 str:"Caysn"];
    if (ret)
        ret = [myApp.myLabel DrawBitmap:100 starty:300 width:50 height:80 style:0 img:[UIImage imageNamed:@"yellowmen.png"] nBinaryAlgorithm:0];
    if (ret)
        ret = [myApp.myLabel PageEnd];
    if (ret)
        ret = [myApp.myLabel PagePrint:1];
